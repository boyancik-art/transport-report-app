...
module.exports=async({page,frame})=>{
 ...
 await frame.locator('#email').fill('runtime-test@example.invalid');await frame.locator('#password').fill('isolated-fixture-only');await frame.locator('#loginForm button').click();await frame.locator('#v444-notice').waitFor({timeout:30000});assert.match(await frame.locator('#v444-notice').innerText(),/Застосунок оновлено до версії v44\.8/);await frame.locator('#v444-ack').click();
