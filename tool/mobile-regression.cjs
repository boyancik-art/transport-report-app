...
(async()=>{
 ...
 for(const file of ['patch-v40-pickup.js','patch-v41-sections.js','patch-v41-mobile-scale.js','patch-v41-route-scale.js','patch-v43-operations.js','patch-v43-1-blocks.js','patch-v43-1-ops-courier.js','patch-v43-1-header.js'])await page.addScriptTag({content:fs.readFileSync(path.join(root,'web',file),'utf8')});
 ...
 const pickupSummary=page.locator('.pk-detail-head + .pk-route');
 assert.ok(await pickupSummary.isVisible());
 assert.ok(await pickupSummary.evaluate(el=>el.getBoundingClientRect().height)>280);
 assert.ok(await pickupSummary.locator('.pk-exp').evaluate(el=>parseFloat(getComputedStyle(el).fontSize))>=18);
 assert.ok(await pickupSummary.locator('.pk-warehouse .pk-val').evaluate(el=>parseFloat(getComputedStyle(el).fontSize))>=18);
 assert.ok(await pickupSummary.evaluate(el=>el.scrollWidth<=el.clientWidth));
 await page.locator('.pk-back').click();await page.waitForSelector('.v431-fop');
 ...
 await page.locator('#v43-from').fill('2026-01-02');await page.locator('#v43-to').fill('2026-01-01');
