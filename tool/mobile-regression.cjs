...
(async()=>{
 ...
 await page.evaluate(()=>{
  ...
  db.source_documents.push(...[{id:201,sale_code:'6136321',pallets:.021,bottles:12,order_amount:2748},{id:202,sale_code:'6136278',pallets:.004,bottles:3,order_amount:576}].map(d=>({...d,document_date:date,route_delivery_id:'P001',customer_id:'pickup-21',address_id:'pickup-address',business_unit:'HoReCa'})));
  db.source_documents.push(...[{id:301,sale_code:'C-3101',pallets:.011,bottles:5,order_amount:3543},{id:302,sale_code:'C-3102',pallets:.021,bottles:9,order_amount:4700}].map(d=>({...d,document_date:date,route_delivery_id:'C001',customer_id:31,business_unit:'HoReCa'})));
 ...
 await page.waitForSelector('.v433-open-delivery');
 assert.equal(await page.locator('#v431-courier [data-point-id="31"] .v436-invoice-table tbody tr').count(),2);
 assert.match(await page.locator('#v431-courier [data-point-id="31"]').innerText(),/Київ, адреса 31/);
 ...
 await page.evaluate(()=>v433Dashboard());await page.waitForSelector('[data-section="stv"]');await page.waitForSelector('#v431-courier');
 assert.deepEqual(await page.locator('#view [data-section]').evaluateAll(els=>els.map(el=>el.dataset.section)),['pickup','stv','sav','fop','courier','replen','bakery']);
 ...
 await page.evaluate(()=>v43OpenRoute(2));await page.locator('.v436-change').click();
 await page.locator('#v436-section').selectOption('STV');await page.evaluate(()=>window.failTable='route_facts');await page.locator('#v436-section-save').click();
 await page.waitForFunction(()=>document.querySelector('#v436-section-error').textContent.includes('Не збережено'));
 assert.equal(await page.locator('#v436-section').inputValue(),'STV');await page.locator('#v436-section-save').click();await page.waitForFunction(()=>!document.body.classList.contains('trts-modal-open'));
 await page.locator('[data-section="stv"] > button').click();
 assert.equal(await page.locator('[data-section="stv"] [data-route-id="2"]').count(),1);
 assert.equal(await page.locator('[data-section="pickup"] [data-route-id="2"]').count(),0);
 assert.equal(await page.evaluate(()=>db.routes.find(r=>r.id===2).expeditor_name),'Самовивіз ТС Плюс');
 await page.evaluate(()=>v43OpenRoute(2));await page.locator('.v436-change').click();await page.locator('#v436-section').selectOption('');await page.locator('#v436-section-save').click();await page.waitForFunction(()=>!document.body.classList.contains('trts-modal-open'));
 await page.waitForSelector('[data-section="pickup"] [data-route-id="2"]');
 assert.equal(await page.evaluate(()=>db.route_facts.find(r=>r.route_id===2).section_override),null);
 await page.evaluate(()=>v43OpenRoute(2));await page.getByRole('button',{name:'Додати адресу',exact:true}).click();await page.locator('#v436-address').fill('Тестова адреса, будинок 5');await page.locator('#v436-address-save').click();await page.waitForFunction(()=>!document.body.classList.contains('trts-modal-open'));
 assert.match(await page.locator('.v43-extra-row').innerText(),/Тестова адреса, будинок 5/);
 await page.evaluate(()=>v433Dashboard());await page.waitForSelector('#v431-courier');
 await page.evaluate(()=>{v431Toggle('fop');v431Toggle('courier');v431Toggle('stv')});await page.evaluate(()=>window.scrollTo(0,0));await capture('approved-dashboard');
 console.log('PASS route block changes persist, retry, reset to imported block, additional address, no imported expeditor change');
