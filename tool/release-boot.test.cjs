...
let x=boot({trts_seen_build:'v44.3',trts_token:'old',trts_refresh:'old-refresh',trts_vault:'{"id":"old","secret":"secret","cipher":"old"}'});assert.equal(x.values.trts_token,undefined);assert.equal(x.values.trts_refresh,undefined);assert.equal(x.values.trts_vault,undefined);assert.equal(x.values.trts_update_notice,'v44.7');assert.equal(x.ctx.window.TRTS_UNLOCKED,false);
x=boot({trts_seen_build:'v44.7',trts_vault:'{"cipher":"current"}'});assert.ok(x.values.trts_vault);assert.equal(x.values.trts_update_notice,undefined);assert.equal(x.ctx.window.TRTS_UNLOCKED,false);
x=boot({});assert.equal(x.values.trts_update_notice,undefined);
x=boot({trts_token:'pre-version-owner'});assert.equal(x.values.trts_token,undefined);assert.equal(x.values.trts_update_notice,'v44.7');
