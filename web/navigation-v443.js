(()=>{
 ...
 window.v445DeleteButton=id=>TRTS_SECURITY.isAdmin()?'<button type="button" class="v445-route-delete v443-danger" aria-label="Видалити маршрут" title="Видалити маршрут" onclick="event.stopPropagation();v443DeleteRoute('+Number(id)+')"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M3 6h18M9 6V3h6v3M5 6l1 15h12l1-15M10 10v7m4-7v7"/></svg>Видалити маршрут</button>':'';
 ...
 function confirmDelete(id){
  if(document.getElementById('v446-confirm'))return Promise.resolve(false);
  return new Promise(resolve=>{
   const focus=document.activeElement,box=document.createElement('dialog');box.id='v446-confirm';
   box.setAttribute('aria-label','Підтвердження видалення маршруту');
   box.innerHTML='<p>Видалити маршрут '+O.E(id)+'? Цю дію неможливо скасувати.</p><div><button type="button" data-confirm="false" autofocus>Скасувати</button><button type="button" data-confirm="true">Видалити</button></div>';
   const finish=value=>{box.close();box.remove();focus?.isConnected&&focus.focus();resolve(value)};
   box.addEventListener('cancel',e=>{e.preventDefault();finish(false)});
   box.addEventListener('click',e=>{const b=e.target.closest('[data-confirm]');if(b)finish(b.dataset.confirm==='true')});
   document.body.append(box);box.showModal();
  });
 }
 let deleting=false;
 window.v443DeleteRoute=async id=>{
  if(deleting||!TRTS_SECURITY.isAdmin())return;const r=O.dat().routes.find(x=>+x.id===+id);if(!r||!(await confirmDelete(r.route_delivery_id)))return;
