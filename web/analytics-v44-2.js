...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.TRTS_ANALYTICS=api})(typeof window==='undefined'?globalThis:window,()=>{
 ...
 function group(lines,key){const groups=new Map();for(const x of lines){const name=x[key];if(!groups.has(name))groups.set(name,[]);groups.get(name).push(x)}return [...groups].map(([name,rows])=>({name,...total(rows)})).sort((a,b)=>a.name.localeCompare(b.name,'uk'))}
