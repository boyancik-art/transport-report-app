...
function extraAllocation(r,x){const s=systemMetrics(r),tt=N(x.tt_count)||1,avg=k=>s.tt?s[k]/s.tt:0,value=k=>N(x[k])>0?N(x[k]):avg(k)*tt;return{pals:value('pallets'),bottles:value('bottles'),weight:value('weight'),tt}}
...
function pointCost(r,p){const rm=allocationMetrics(r),pm={pals:N(p.pallets),bottles:N(p.bottles),weight:N(p.weight),tt:1},ds=pointDocs(r,p);pm.pals=ds.reduce((s,x)=>s+N(x.pallets),0)||pm.pals;pm.bottles=ds.reduce((s,x)=>s+N(x.bottles),0)||pm.bottles;pm.weight=ds.reduce((s,x)=>s+N(x.weight),0)||pm.weight;const sum=ds.reduce((s,x)=>s+N(x.order_amount),0)||N(p.order_amount),cost=routeCost(r)*allocationShare(pm,rm);return{...pm,sum,cost,log:sum?cost/sum*100:0}}
...
function addressCard(r,p,i){
 ...
 return `<article class="v43-tt v436-address" data-point-id="${p.id}" onclick="v43OpenTT(${r.id},${p.id})"><button type="button" class="v436-address-head" onclick="event.stopPropagation();v43OpenTT(${r.id},${p.id})"><span class="v436-number">${i+1}</span><span><b>${E(customer)}</b><small>${E(pointAddress(r,p))}</small></span>${uiIcon('chevron')}</button><div class="v436-business"><div><small>Business</small><b>${E(biz(r,p))}</b></div><div><small>ТА</small><b>${E(taName(r,p))}</b></div></div><div class="v436-point-metrics"><span>Пал <b>${F(c.pals,3)}</b></span><span>Пляшки <b>${F(c.bottles)}</b></span><span>Вага <b>${F(c.weight,1)} кг</b></span><span class="v434-doc-count">К-ть накладних <b>${F(invoiceCount(p,ds))}</b></span></div>${isFop(r)?`<div class="v437-point-finance"><span>Доставка ТТ <b>${M2(c.cost)}</b></span><span>% логістики <b>${P(c.log)}</b></span></div>`:''}${invoiceTable(r,p)}</article>`;
}
window.v436AddressCard=addressCard;
window.v43OpenRoute=id=>{
 const r=(dat().routes||[]).find(x=>+x.id===+id);if(!r)return;screenLayout(true);
 const ps=points(r);
 v().innerHTML=`<div class="v43-screen v436-detail"><div class="v436-detail-head"><button class="v43-back" aria-label="Назад до маршрутів" onclick="v433Dashboard()">${uiIcon('back')}</button><div><h2>Маршрут</h2><small>#${E(r.route_delivery_id||r.id)}</small></div></div>${routeSummary(r,true)}<div class="v436-address-title"><h3>Адреси маршруту</h3><button type="button" onclick="v43AddTT(${r.id})">Додати ТТ ${uiIcon('plus')}</button></div>${ps.map((p,i)=>addressCard(r,p,i)).join('')}${extraFor(r).map(x=>{const c=extraCost(r,x),pickup=isPickup(r);return`<article class="v43-extra-row v437-extra-card"><div><small>Додаткова ТТ</small><b>${E(x.name)}</b></div><div class="v437-extra-metrics"><span>ТТ <b>${c.tt}</b></span><span>Пал <b>${F(N(x.pallets),3)}</b></span><span>Пляшки <b>${F(N(x.bottles))}</b></span><span>Вага <b>${F(N(x.weight),1)} кг</b></span>${pickup?'':`<span>Частка доставки <b>${M2(c.cost)}</b></span><span>Вартість 1 ТТ <b>${M2(c.costTT)}</b></span>`}</div></article>`}).join('')}<button type="button" class="v436-bottom primary" onclick="v436ChangeSection(${r.id})">${uiIcon('swap')} Змінити блок маршруту</button></div>`;
 window.scrollTo(0,0);
};
window.v412Route=id=>window.v43OpenRoute(id);
window.v436ChangeSection=id=>{
 const r=(dat().routes||[]).find(x=>+x.id===+id);if(!r)return;
 const choices=['Самовивіз','ФОП','TS','Кур’єр'];
...
async function init(){if(typeof api!=='function'||!v()||!window.TRTS_UI)return setTimeout(init,200);window.logistics=()=>{periodBar();renderDashboard()};if(window.TRTS_MODERN_RUNTIME)window.go=window.home=window.analytics=window.ai=window.more=window.v39Section=window.logistics;periodBar();try{await loadRange()}catch(e){console.error(e)}badge()}
setTimeout(init,180);
})();
