...
async function loadRange(){
 ...
 try{
  ...
  const [pts,ff,dd,ee,pp,cc,wm,gg,mm,rr,ru]=await Promise.all([
   ...
   apiSafe('/rest/v1/employee_directory?select=employee_id,employee_name'),
   apiSafe('/rest/v1/transport_carriers?select=id,name,carrier_type&active=eq.true&order=name'),
   apiSafe('/rest/v1/warehouse_display_map?select=source_warehouse,display_name&active=eq.true'),
...
function routeSummary(r,detail=false){
 const f=facts(r),m=metrics(r),pickup=isPickup(r),fop=isFop(r),carrier=T(f.carrier_name)||'Не вказано',wave=T(f.wave)||'Не вказано',bad=fop&&!/ТОВ ТС ПЛЮС/i.test(carrier)&&m.cost<=0;
 const edit=`event.stopPropagation();v431EditTransport(${r.id})`;
 ...
 return `<article class="v436-route ${detail?'v43-route-detail':pickup?'v431-pickup-card':'v43-route'} ${bad?'warn':''}" ${detail?'':`onclick="v43OpenRoute(${r.id})"`} data-route-id="${r.id}"><div class="v436-route-top">${selectGroup&&fop&&!detail?`<input type="checkbox" aria-label="Вибрати маршрут ${E(r.route_delivery_id)}" ${selected.has(r.id)?'checked':''} onclick="event.stopPropagation();v43SelectRoute(${r.id},this.checked)">`:''}<time>${E(displayDate(r.route_date||today()))}</time><b class="v436-route-id">${detail?'ID ':''}${E(r.route_delivery_id||r.id)}</b>${detail?`<button type="button" class="v436-change" onclick="v436ChangeSection(${r.id})">${uiIcon('edit')}<span>Змінити блок</span></button>`:bad?pill('Без тарифу','bad'):''}</div><div class="v436-exp">${uiIcon('person')}<div><small>Експедитор</small><b>${E(r.expeditor_name||'—')}</b></div><button type="button" class="v436-swap" aria-label="Змінити блок маршруту" onclick="event.stopPropagation();v436ChangeSection(${r.id})">${uiIcon('swap')}</button></div><div class="v436-route-fields ${!detail&&pickup?'compact':''}">${field('warehouse','Склад відвантаження',displayWh(r.warehouse),'','v431-cell')}${detail||!pickup?field('truck','Перевізник',carrier,edit):''}${detail||!pickup?field('wave','Хвиля',/^\d+$/.test(wave)?wave+' год':wave,edit,'v436-wave'):''}<div class="v436-metrics"><div><small>ТТ</small><b>${m.tt}</b></div><div><small>Пал</small><b>${F(m.pals,3)}</b></div><div><small>Пляшки</small><b>${F(m.bottles)}</b></div></div></div>${fop?`<div class="v436-finance"><div><small>${f.tariff_group_id?'Частка тарифу':'Тариф'}</small><b>${M(m.cost)}</b></div><div><small>% логістики</small><b>${P(m.log)}</b></div></div>`:''}<div class="v436-total"><small>Сума маршруту</small><b>${M(m.sales)}</b></div>${fop?`<button class="v431-edit-transport v433-edit" onclick="${edit}">Перевізник · хвиля · тариф</button>${deliveredButton(r,detail?'route':'dashboard')}`:''}</article>`;
}
...
function addressCard(r,p,i){
 const ds=pointDocs(r,p),pickup=isPickup(r),customer=T(p.customer_name)||T(ds[0]?.customer_name)||'Клієнт',c=pointCost(r,p);
 return `<article class="v43-tt v436-address" data-point-id="${p.id}" onclick="v43OpenTT(${r.id},${p.id})"><button type="button" class="v436-address-head" onclick="event.stopPropagation();v43OpenTT(${r.id},${p.id})"><span class="v436-number">${i+1}</span><span><b>${E(customer)}</b><small>${E(pointAddress(r,p))}</small></span>${uiIcon('chevron')}</button><div class="v436-business"><div><small>Business</small><b>${E(biz(r,p))}</b></div><div><small>ТА</small><b>${E(taName(r,p))}</b></div></div>${!pickup?`<div class="v436-point-metrics"><span>Пал <b>${F(c.pals,3)}</b></span><span>Пляшки <b>${F(c.bottles)}</b></span><span>КГ <b>${F(c.weight,1)}</b></span><span class="v434-doc-count">К-ть накладних <b>${F(invoiceCount(p,ds))}</b></span></div>`:''}${invoiceTable(r,p)}</article>`;
}
window.v436AddressCard=addressCard;
window.v43OpenRoute=id=>{
 const r=(dat().routes||[]).find(x=>+x.id===+id);if(!r)return;
 const ps=points(r);
 v().innerHTML=`<div class="v43-screen v436-detail"><div class="v436-detail-head"><button class="v43-back" aria-label="Назад до маршрутів" onclick="v433Dashboard()">${uiIcon('back')}</button><div><h2>Маршрут</h2><small>#${E(r.route_delivery_id||r.id)}</small></div></div>${routeSummary(r,true)}<div class="v436-address-title"><h3>Адреси маршруту</h3><button type="button" onclick="v436AddAddress(${r.id})">Додати адресу ${uiIcon('plus')}</button></div>${ps.map((p,i)=>addressCard(r,p,i)).join('')}${extraFor(r).map(x=>`<div class="v43-extra-row"><b>${E(x.name)}</b><span>${N(x.tt_count)||1} ТТ</span></div>`).join('')}<button type="button" class="v436-bottom primary" onclick="v436ChangeSection(${r.id})">${uiIcon('swap')} Змінити блок маршруту</button></div>`;
 window.scrollTo(0,0);
};
window.v412Route=id=>window.v43OpenRoute(id);
window.v436ChangeSection=id=>{
 const r=(dat().routes||[]).find(x=>+x.id===+id);if(!r)return;
 const choices=['Самовивіз','STV','SAV','ФОП','TS','Кур’єр','Пекарня/Фреш'];
...
window.v433EditRoute=async id=>{
 ...
 try{
 const waves=await api('/rest/v1/transport_waves?select=name&active=eq.true&order=name');
 const names=[...new Set([...meta.carriers.map(x=>x.name),f.carrier_name].filter(Boolean))],wn=[...new Set([...waves.map(x=>x.name),f.wave].filter(Boolean))];
...
window.v435EditDelivered=(id,back='route')=>{
 const r=(dat().routes||[]).find(x=>+x.id===+id);if(!r||!isFop(r))return;
...
async function init(){if(typeof api!=='function'||!v()||!window.TRTS_UI)return setTimeout(init,200);periodBar();try{await loadRange()}catch(e){console.error(e)}window.logistics=()=>{periodBar();renderDashboard()};badge()}
setTimeout(init,180);
})();
