...
function routePointCount(r){return points(r).length+extraFor(r).reduce((a,x)=>a+(N(x.tt_count)||1),0)}
...
function manualCard(x){return`<article class="v43-route manual"><div class="v43-route-top"><div><small>${E(x.route_date)}</small><b>Створено вручну</b></div>${pill(x.purpose,'group')}${window.TRTS_SECURITY?.isAdmin()?`<button class="v443-delete v443-danger" onclick="v443DeleteManualRoute(${x.id})">Видалити маршрут</button>`:''}</div><div class="v43-four"><div><small>ТТ</small><b>${N(x.tt_count)}</b></div><div><small>Перевізник</small><b>${E(x.carrier_name||'—')}</b></div><div><small>Водій</small><b>${E(x.driver_name||'—')}</b></div><div><small>Авто</small><b>${E(x.vehicle_number||'—')}</b></div></div><div class="v43-sales"><small>Тариф</small><b>${M(window.TRTS_COSTS.own(x.carrier_name)?0:x.tariff)}</b></div>${window.TRTS_COSTS.own(x.carrier_name)?`<div class="v43-sales"><small>Витрати власного парку</small><b>${M2(window.TRTS_FINANCE.fleetCost('m:'+x.id))}</b></div>`:''}</article>`}
window.v433Dashboard=()=>renderDashboard();
function renderDashboard(){window.TRTS_APP?.selectRoutes();const vv=v();if(!vv)return;screenLayout();let rs=(dat().routes||[]).filter(isFop).filter(r=>window.TRTS_APP?.routeMatches('fop',r)??true);if(filter==='notariff')rs=rs.filter(r=>!/ТОВ ТС ПЛЮС/i.test(T(facts(r).carrier_name))&&routeCost(r)<=0);const renderCards=batch=>{if(!groupExp)return batch.map(routeCard).join('');const groups=new Map();batch.forEach(r=>{const k=T(r.expeditor_name)||'Без експедитора';if(!groups.has(k))groups.set(k,[]);groups.get(k).push(r)});return [...groups].map(([k,a])=>`<div class="v43-exp-group"><h3>${E(k)} <span>${rs.filter(r=>(T(r.expeditor_name)||'Без експедитора')===k).length}</span></h3>${a.map(routeCard).join('')}</div>`).join('')};const cards=window.TRTS_ROUTE_PAGES.html('fop',rs,renderCards);const pick=(dat().routes||[]).filter(isPickup);vv.innerHTML=`<div class="v43-screen"><section class="v43-head"><div><h2>ФОП / TS</h2><small>${E(statusText())}</small></div><div class="v43-actions"><button onclick="v43CarrierDirectory()">Перевізники</button><button class="primary" onclick="v43ManualRoute()">+ Створити маршрут</button></div></section><div class="v43-filters"><button class="${filter==='all'?'on':''}" onclick="v43Filter('all')">Усі · ${rs.length}</button><button class="${filter==='notariff'?'on':''}" onclick="v43Filter('notariff')">Авто без тарифу</button><button class="${groupExp?'on':''}" onclick="v43GroupExp()">Групувати по експедиторам</button></div><div class="v43-groupbar"><button onclick="v43StartGroup()">${selectGroup?'Скасувати групування':'Об’єднати експедиторів · 1 тариф'}</button>${selectGroup?`<button class="primary" ${selected.size<2?'disabled':''} onclick="v43FinishGroup()">Об’єднати експедиторів (${selected.size})</button>`:''}</div><div class="v43-stack">${cards||'<div class="v43-empty">Маршрутів за вибраний період немає</div>'}${meta.manual.filter(x=>(x.block||'fop')==='fop').filter(x=>window.TRTS_APP?.manualMatches('fop',x)??true).map(manualCard).join('')}</div><section class="v43-section"><div class="v43-section-title"><div><h2>Поповнення філій</h2><small>${meta.replenishments.length} за період</small></div><button class="primary" onclick="v43Replenishment()">+ Додати поповнення</button></div>${window.TRTS_RELEASE?.replenTotal(meta.replenishments)||''}${meta.replenishments.map(replenCard).join('')||'<div class="v43-empty">Поповнень за період немає</div>'}</section><section class="v43-section"><div class="v43-section-title"><h2>Самовивіз</h2><small>${pick.length}</small></div></section></div>`;badge()}
...
function carrierOptions(sel='',bakery=false){const names=window.TRTS_FINANCE?.carrierNames(bakery?'bakery':'fop')||meta.carriers.map(x=>x.name).filter(x=>!/^Нова\\s*Пошта$/i.test(x));return names.map(name=>`<option ${name===sel?'selected':''}>${E(name)}</option>`).join('')}
window.v43ManualRoute=(block='fop')=>window.v44ManualRoute(block);
window.v43FinishGroup=()=>{if(selected.size<2)return;pendingGroup=null;modal('Один тариф на декілька експедиторів',`<div class="v43-form"><div class="v43-info">Вибрано маршрутів експедиторів: <b>${selected.size}</b>. Тариф буде розподілений між усіма ТТ цих маршрутів за 30% палети / 50% пляшки / 20% кг.</div><label>Перевізник<select id="tg-carrier" onchange="v44OwnTariff('tg-carrier','tg-tariff')">${carrierOptions('',groupSection==='bakery')}</select></label><label>Водій<input id="tg-driver"></label><label>Авто<input id="tg-vehicle"></label><label>Загальний тариф<input id="tg-tariff" ${groupSection==='bakery'?'value="0" readonly':''} type="number" min="0" step="0.01" required></label><p id="tg-error" class="v433-error" role="alert"></p></div>`,'<button onclick="v43CloseModal()">Скасувати</button><button id="tg-save" class="primary" onclick="v43SaveGroup()">Об’єднати</button>')};
...
window.v43SaveGroup=async()=>{
 ...
 const payload={group_date:dates[0],carrier_name:$('#tg-carrier').value,driver_name:T($('#tg-driver').value),vehicle_number:T($('#tg-vehicle').value),tariff:window.TRTS_COSTS?.own($('#tg-carrier').value)?0:N($('#tg-tariff').value)};
...
function pointMeasure(r,p){const ds=pointDocs(r,p);return{id:p.id,pals:ds.reduce((s,x)=>s+N(x.pallets),0)||N(p.pallets),bottles:ds.reduce((s,x)=>s+N(x.bottles),0)||N(p.bottles),weight:ds.reduce((s,x)=>s+N(x.weight),0)||N(p.weight),tt:1,sum:ds.reduce((s,x)=>s+N(x.order_amount),0)||N(p.order_amount)}}
const allocationCache=new WeakMap();
function routeAllocation(r){const cost=routeCost(r),ex=extraFor(r),signature=cost+'|'+JSON.stringify(ex);const prev=allocationCache.get(r);if(prev?.signature===signature)return prev.shares;const items=[...points(r).map(p=>pointMeasure(r,p)),...ex.map(x=>({id:'extra:'+x.id,...extraAllocation(r,x)}))],shares=new Map((items.length?window.TRTS_COSTS.split(cost,items):[]).map(x=>[String(x.id),x.cost]));allocationCache.set(r,{signature,shares});return shares}
function pointCost(r,p){const pm=pointMeasure(r,p),cost=window.TRTS_FINANCE?.pointCost(r,p)??(sectionKey(r)==='courier'?N(window.v439CourierPointCost?.(p)):routeAllocation(r).get(String(p.id))||0);return{...pm,cost,log:pm.sum?cost/pm.sum*100:0}}
function extraCost(r,x){const q=extraAllocation(r,x),cost=routeAllocation(r).get('extra:'+x.id)||0;return{...q,cost,costTT:q.tt?cost/q.tt:0}}
...
function routeSummary(r,detail=false){
 const f=facts(r),m=metrics(r),pickup=isPickup(r),fop=isTariff(r),partner=['stv','sav'].includes(sectionKey(r)),courier=sectionKey(r)==='courier',carrier=(partner?sectionKey(r).toUpperCase():courier?window.v439CourierDefault?.(r):T(f.carrier_name))||'Не вказано',wave=T(f.wave)||'Не вказано',bad=fop&&!/ТОВ ТС ПЛЮС/i.test(carrier)&&m.cost<=0;
 if(courier){m.cost=window.v439CourierRouteCost?.(r)||0;m.costTT=m.tt?m.cost/m.tt:0;m.log=m.sales?m.cost/m.sales*100:0}
 const edit=partner?'':courier?`event.stopPropagation();v439EditCourierCarrier(${r.id})`:`event.stopPropagation();v431EditTransport(${r.id})`;
 ...
 return `<article class="v436-route v446-route ${detail?'v43-route-detail':pickup?'v431-pickup-card':'v43-route'} ${bad?'warn':''}" ${detail?'':`onclick="v43OpenRoute(${r.id})"`} data-route-id="${r.id}"><div class="v436-route-top">${selectGroup&&fop&&sectionKey(r)===groupSection&&!detail?`<input type="checkbox" aria-label="Вибрати експедитора маршруту ${E(r.route_delivery_id)}" ${selected.has(r.id)?'checked':''} onclick="event.stopPropagation();v43SelectRoute(${r.id},this.checked)">`:''}<time>${E(displayDate(r.route_date||today()))}</time><b class="v436-route-id">${detail?'ID ':''}${E(r.route_delivery_id||r.id)}</b>${detail?`<button type="button" class="v436-change" onclick="v436ChangeSection(${r.id})">${uiIcon('edit')}<span>Змінити блок</span></button>`:bad?pill('Без тарифу','bad'):''}</div><div class="v436-exp">${uiIcon('person')}<div><small>Експедитор</small><b>${E(r.expeditor_name||'—')}</b></div><button type="button" class="v436-swap" aria-label="Змінити блок маршруту" onclick="event.stopPropagation();v436ChangeSection(${r.id})">${uiIcon('swap')}</button></div>${!detail?(window.v445DeleteButton?.(r.id)||''):''}<div class="v436-route-fields ${!detail&&pickup?'compact':''}">${field('warehouse','Склад відвантаження',displayWh(r.warehouse),'','v431-cell')}${!pickup&&window.TRTS_APP?field('warehouse','Філія покриття',window.TRTS_UI446?.coverage(r)||window.TRTS_APP.routeBranch(r)):''}${detail||!pickup?field('truck','Перевізник',carrier,edit):''}${!courier&&!partner&&(detail||!pickup)?field('wave','Хвиля',/^\d+$/.test(wave)?wave+' год':wave,edit,'v436-wave'):''}<div class="v436-metrics"><div><small>ТТ</small><b>${m.tt}</b></div><div><small>Пал</small><b>${F(m.pals,3)}</b></div><div><small>Пляшки</small><b>${F(m.bottles)}</b></div><div><small>Вага</small><b>${F(m.weight,1)} кг</b></div></div></div>${window.TRTS_UI446?.tariff(r)||''}${fop||courier?`<div class="v436-finance"><div><small>${window.TRTS_COSTS?.own(carrier)?'Тариф':f.tariff_group_id?'Частка тарифу':'Тариф'}</small><b>${M(window.TRTS_COSTS.own(carrier)?0:m.cost)}</b></div><div><small>% логістики</small><b>${P(m.log)}</b></div></div>`:''}${window.TRTS_RELEASE?.routeMetrics(r)||''}${window.TRTS_FINANCE?.routeFields(r)||''}<div class="v436-total"><small>Сума маршруту</small><b>${M(m.sales)}</b></div>${fop?`<button class="v431-edit-transport v433-edit" onclick="${edit}">Перевізник · хвиля · тариф</button>${deliveredButton(r,detail?'route':'dashboard')}`:''}</article>`;
}
...
window.v43OpenTT=(rid,pid)=>{const r=(dat().routes||[]).find(x=>+x.id===+rid),p=r&&points(r).find(x=>+x.id===+pid);if(!r||!p)return;screenLayout(true);window.scrollTo(0,0);const c=pointCost(r,p),ds=pointDocs(r,p),inv=invoiceGroups(ds),count=invoiceCount(p,ds),pickup=isPickup(r)||Boolean(window.TRTS_FINANCE?.quote(r,p)?.errors.length),distributed=inv.length?window.TRTS_COSTS.split(c.cost,inv.map((x,i)=>({...x,id:i,tt:1}))):[],tot=inv.reduce((a,x)=>{a.pals+=x.pals;a.bottles+=x.bottles;a.weight+=x.weight;return a},{pals:0,bottles:0,weight:0});v().innerHTML=`<div class="v43-screen"><button class="v43-back" onclick="v43OpenRoute(${r.id})">‹ Маршрут</button><section class="v43-route-detail"><h2>${E(p.customer_name||ds[0]?.customer_name||'ТТ')}</h2><p class="v435-tt-address">${E(pointAddress(r,p))}</p>${sectionKey(r)==='courier'?`<button class="v439-carrier-tt" onclick="v439EditCourierCarrier(${r.id},${p.id},true)">Перевізник ТТ · ${E(window.v439CourierPointCarrier?.(r,p)||'Не вказано')}</button><button class="primary" onclick="v433OpenDelivery(${r.id},${p.id})">Тариф ТТ · об’єднання</button>`:''}${window.TRTS_FINANCE?.pointFields(r,p)||''}${window.TRTS_APP?.pointBranchField(r,p)||''}${window.TRTS_RELEASE?.pointMetrics(r,p)||''}${!['fop','bakery','courier'].includes(sectionKey(r))?`<div class="v447-point-sum"><small>Сума документів ТТ</small><b>${M2(c.sum)}</b></div>`:''}${invoiceCountRow(p,ds)}<div class="v43-two"><div><small>Business</small><b>${E(biz(r,p))}</b></div><div><small>ТА</small><b>${E(taName(r,p))}</b></div></div><div class="v43-four"><div><small>Пал</small><b>${F(c.pals,3)}</b></div><div><small>Пляшки</small><b>${F(c.bottles)}</b></div><div><small>КГ</small><b>${F(c.weight,1)}</b></div>${pickup||['stv','sav'].includes(sectionKey(r))?'':`<div><small>% лог.</small><b>${P(c.log)}</b></div>`}</div></section><h3 class="v43-subtitle">НАКЛАДНІ В ЦІЙ ТТ · ${F(count)}</h3>${inv.length<count?`<div class="v43-empty">Завантажено ${F(inv.length)} із ${F(count)} накладних. Оновіть дані, щоб побачити весь список.</div>`:count===0?'<div class="v43-empty">Накладних у цій ТТ немає.</div>':''}${inv.map((x,i)=>{const cost=distributed[i]?.cost||0,log=x.sum?cost/x.sum*100:0;return`<article class="v43-invoice v439-invoice" data-invoice-index="${i}"><header><span class="v439-invoice-icon">${uiIcon('box')}</span><div><small>ВИДАТКОВА НАКЛАДНА</small><b>№ ${E(x.no)}</b></div></header><div class="v43-four"><div><small>Пляшки</small><b>${F(x.bottles)}</b></div><div><small>КГ</small><b>${F(x.weight,1)}</b></div><div><small>Пал</small><b>${F(x.pals,3)}</b></div>${pickup?'':`<div><small>% лог.</small><b>${P(log)}</b></div>`}</div><div class="v43-two"><div><small>Сума</small><b>${M(x.sum)}</b></div>${pickup?'':`<div><small>Доставка</small><b>${M2(cost)}</b></div>`}</div></article>`}).join('')}</div>`};
...
window.v433EditRoute=async id=>{
 ...
 try{
 const names=window.TRTS_FINANCE.carrierNames(isBakery(r)?'bakery':'fop'),wn=isBakery(r)?BAKERY_OPTIONS:FOP_WAVES;
 const options=(a,sel)=>'<option value="">Оберіть</option>'+a.map(x=>`<option value="${E(x)}" ${x===sel?'selected':''}>${E(x)}</option>`).join('');
 modal('Перевізник, хвиля і тариф',`<form id="v433-route-form" class="v43-form" onsubmit="event.preventDefault();v433SaveRoute(${id})"><label>Перевізник<select id="v433-carrier" onchange="v44OwnTariff('v433-carrier','v433-tariff')" required>${options(names,f.carrier_name)}</select></label>${window.v44CarrierField('v433-carrier',isBakery(r)?'bakery':'fop')}<label>Хвиля<select id="v433-wave" required>${options(wn,f.wave)}</select></label><label>${group?'Загальний тариф групи':'Тариф'}, грн<input id="v433-tariff" type="number" min="0" step="0.01" inputmode="decimal" value="${group?N(group.tariff):f.corrected_tariff??f.tariff??''}" required></label>${group?'<p class="v433-full">Спільний тариф усіх маршрутів цієї групи. Частка маршруту перерахується після збереження.</p>':''}<p id="v433-route-error" class="v433-error" role="alert"></p></form>`,'<button onclick="v43CloseModal()">Скасувати</button><button id="v433-route-save" class="primary" onclick="v433SaveRoute('+id+')">Готово</button>');window.v44OwnTariff('v433-carrier','v433-tariff');
...
window.v433SaveRoute=async id=>{
 ...
 const tariff=window.TRTS_COSTS.own($('#v433-carrier').value)?0:N($('#v433-tariff').value),payload={carrier_name:$('#v433-carrier').value,wave:$('#v433-wave').value};
 ...
 await window.TRTS_FINANCE?.refreshFleet();v43CloseModal();v43OpenRoute(id);
...
window.v439SaveVT=async id=>{
 ...
 try{
  ...
  meta.extras=[...meta.extras.filter(x=>+x.route_id!==+id),...saved];await window.TRTS_FINANCE?.refreshFleet();v43CloseModal();v43OpenRoute(id);
