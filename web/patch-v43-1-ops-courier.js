...
window.v431Toggle=key=>{if(!(key in blockState))return;blockState[key]=!blockState[key];localStorage[key==='base'?'v442_base':'v431_fast_'+key]=blockState[key]?'1':'0';applyBlock(key)};
...
let cdata={shipments:[],links:[],carriers:[]},loading=false,loadedSource=null,loadedKey='';
async function loadCourier(){
 const source=dat().routes,period=window.v435ReportPeriod?.(),key=JSON.stringify(period);
 if(loading){await loading;if(loadedSource!==dat().routes||loadedKey!==JSON.stringify(window.v435ReportPeriod?.()))return loadCourier();return}
 loading=(async()=>{const cr=(source||[]).filter(isCourier),ids=cr.map(r=>r.id),rid=ids.length?`(${ids.join(',')})`:'(0)',dates=cr.map(r=>r.route_date).filter(Boolean).sort(),from=period?.from||dates[0]||new Date().toISOString().slice(0,10),to=period?.to||dates.at(-1)||from,read=window.v435ReadRange||api;
 const [s,l,c]=await Promise.all([read(`/rest/v1/courier_shipments?select=*&shipment_date=gte.${from}&shipment_date=lte.${to}&order=created_at.desc`),window.TRTS_OPS.byIds('courier_shipment_points','route_id',ids),api('/rest/v1/courier_carriers?select=*&active=eq.true&order=name')]);
 if(source===dat().routes&&key===JSON.stringify(window.v435ReportPeriod?.())){cdata={shipments:s||[],links:l||[],carriers:c||[]};loadedSource=source;loadedKey=key}
 })();try{await loading}finally{loading=false}
 if(source!==dat().routes||key!==JSON.stringify(window.v435ReportPeriod?.()))return loadCourier();
}

async function readSnapshot(dataset,from,to){const [shipments,links,carriers]=await Promise.all([window.TRTS_OPS.apiRange('/rest/v1/courier_shipments?select=*&shipment_date=gte.'+from+'&shipment_date=lte.'+to+'&order=created_at.desc'),window.TRTS_OPS.byIds('courier_shipment_points','route_id',dataset.routes.map(r=>r.id)),api('/rest/v1/courier_carriers?select=*&active=eq.true&order=name')]);return{shipments,links,carriers}}
function withSnapshot(snapshot,fn){const old=cdata;cdata=snapshot;try{const result=fn();if(result?.then)throw Error('Synchronous courier snapshot required');return result}finally{cdata=old}}
window.TRTS_COURIER={readSnapshot,withSnapshot,load:loadCourier,snapshot:()=>cdata};
...
window.v439CourierPointCost=p=>{
 const link=linkFor(p);if(!link)return 0;
 const members=cdata.links.filter(x=>T(x.shipment_id)===T(link.shipment_id)),ids=members.map(x=>+x.route_point_id);
 const eligible=ids.filter(id=>{const point=(dat().points||[]).find(x=>+x.id===id),r=(dat().routes||[]).find(x=>+x.id===+point?.route_id);return point&&!window.TRTS_COSTS.own(window.v439CourierPointCarrier(r,point))});
 if(!eligible.includes(+p.id))return 0;
 const total=members.reduce((sum,x)=>sum+N(x.delivery_cost),0);return shares(total,eligible).find(x=>+x.id===+p.id)?.cost||0;
};
...
async function renderCourier(){if(renderingCourier)return;const screen=$('.v43-screen');if(!screen?.querySelector('.v43-stack'))return;renderingCourier=true;try{await loadCourier();if(!screen.isConnected||screen!==$('.v43-screen'))return;let sec=$('#v431-courier');if(sec)sec.remove();const routes=(dat().routes||[]).filter(isCourier).filter(r=>window.TRTS_APP?.routeMatches('courier',r)??true),totalTT=routes.reduce((s,r)=>s+points(r).length,0),sales=routes.reduce((s,r)=>s+points(r).reduce((a,p)=>a+pointSales(r,p),0),0),cost=routes.reduce((s,r)=>s+window.v439CourierRouteCost(r),0),log=sales?cost/sales*100:0;sec=document.createElement('section');sec.id='v431-courier';sec.className='v431-block';sec.dataset.section='courier';sec.innerHTML=`<button class="v431-courier-head" onclick="v431CourierToggle()"><div><b>${window.TRTS_SHELL?.icon('courier')||window.TRTS_UI?.icon('box')||''}Кур’єрські відправлення · ${routes.length}</b><small>${totalTT} ТТ</small></div><span class="v431-toggle-label">Згорнути ︿</span></button><div class="v431-courier-body">${window.TRTS_APP?.carrierFilter('courier')||''}<div class="v431-cactions"><button onclick="v431CourierCarriers()">Перевізники</button><button class="primary" onclick="v431CreateCourier()">Відкрити доставку</button></div><div class="v431-ckpi"><div><small>ТТ</small><b>${totalTT}</b></div><div><small>Накладних</small><b>${routes.reduce((sum,r)=>sum+points(r).reduce((n,p)=>n+(window.v436InvoiceCount?.(r,p)||0),0),0)}</b></div><div><small>Витрати</small><b>${M(cost)}</b></div><div><small>% логістики</small><b>${P(log)}</b></div></div>${window.TRTS_ROUTE_PAGES.html('courier',routes,items=>items.map(courierRouteCard).join(''))}</div>`;const anchor=$('#v436-courier-anchor');if(anchor)anchor.after(sec);else screen.appendChild(sec);applyBlock('courier')}catch(e){console.error('courier',e);if(!$('#v433-courier-load-error')){const err=document.createElement('p');err.id='v433-courier-load-error';err.className='v433-error';err.textContent='Не вдалося завантажити кур’єрку: '+e.message;screen.appendChild(err)}}finally{renderingCourier=false;const current=$('.v43-screen');if(current!==screen&&current?.querySelector('.v43-stack')&&!$('#v431-courier')&&!$('#v433-courier-load-error'))setTimeout(renderCourier,0)}}
...
const groupOwn=g=>g.ids.every(id=>window.TRTS_COSTS.own(T(linkFor(pointById(id))?.carrier_override)||draft.carrier));
...
window.v433CourierCarrier=value=>{if(draft){draft.carrier=value;draft.groups.forEach(g=>{if(groupOwn(g))g.tariff='0'});renderDraft()}};
...
window.v433ChooseShared=key=>{
 ...
 box.innerHTML=`<div class="v433-shared-options"><p class="v431-note">Обрані ТТ перейдуть під цей спільний тариф. Якщо ТТ уже входить у групу, буде додано всю її групу; старий тариф цієї групи буде замінено.</p>${rows.map(p=>`<label><input type="checkbox" name="v433-shared-${g.key}" value="${p.id}" ${g.ids.includes(+p.id)?'checked':''} ${g.ids.includes(+p.id)?'disabled':''}><span>${pointLabel(+p.id)}</span></label>`).join('')}<button class="primary" onclick="v433ApplyShared(${g.key})">Готово</button></div>`;
};
window.v433ApplyShared=key=>{
 const g=draft?.groups.find(x=>x.key===+key);if(!g)return;
 const chosen=[...document.querySelectorAll('input[name="v433-shared-'+key+'"]:checked')].map(x=>+x.value),ids=new Set(g.ids);
 for(const id of chosen){
  const local=draft.groups.find(x=>x.ids.includes(id)),link=cdata.links.find(x=>+x.route_point_id===id);
  for(const member of local?local.ids:link?cdata.links.filter(x=>T(x.shipment_id)===T(link.shipment_id)).map(x=>+x.route_point_id):[id])ids.add(member);
 }
 g.ids=[...ids];draft.groups=draft.groups.filter(x=>x===g||!x.ids.some(id=>ids.has(id)));
 renderDraft();
};
window.v431CreateCourier=()=>{
 const routes=(dat().routes||[]).filter(isCourier);
 showModal('Оберіть доставку',routes.map(r=>`<button class="v433-delivery-choice" onclick="v433OpenDelivery(${r.id})">${E(r.route_delivery_id||r.id)}<small>${E(r.expeditor_name)} · <time datetime="${E(r.route_date)}">${E(r.route_date.split("-").reverse().join("."))}</time> · ${points(r).length} ТТ</small></button>`).join('')||'<p>Кур’єрських доставок за період немає.</p>','<button onclick="v431CloseModal()">Готово</button>');
};
window.v431SaveCourier=async()=>{
 ...
 for(const g of groups){for(const id of g.ids){const link=latest.find(l=>+l.route_point_id===id);const expected=draft.expectedLinks.find(x=>+x.route_point_id===id);if(link&&T(link.shipment_id)!==T(g.sid)&&T(link.shipment_id)!==T(expected?.shipment_id))throw Error('ТТ змінена в іншій сесії. Відкрийте доставку знову')}}
 for(const g of groups){
  ...
  const paid=g.ids.filter(id=>!window.TRTS_COSTS.own(T(linkFor(pointById(id))?.carrier_override)||draft.carrier)),calculated=paid.length?shares(g.tariff,paid):[],allocation=g.ids.map(id=>({id,cost:calculated.find(x=>x.id===id)?.cost||0})),add=[];
  for(const item of allocation){const link=latest.find(l=>+l.route_point_id===item.id);if(link){const updated=await api('/rest/v1/courier_shipment_points?id=eq.'+link.id+'&shipment_id=eq.'+encodeURIComponent(link.shipment_id)+'&delivery_cost=eq.'+N(link.delivery_cost),{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({shipment_id:g.sid,delivery_cost:item.cost})});if(!updated?.length)throw Error('Тариф ТТ змінено в іншій сесії');Object.assign(link,updated[0])}else add.push({shipment_id:g.sid,route_id:routeByPoint(item.id).id,route_point_id:item.id,delivery_cost:item.cost})}
...
function enhance(){addFopEditors();['base','fop','pickup','stv','sav','bakery','other','replen'].forEach(applyBlock);const pick=findBlock('pickup');if(pick)pick.classList.add('v431-pickup-purple');if($('.v43-stack')&&!$('#v431-courier')&&!$('#v433-courier-load-error'))renderCourier()}
