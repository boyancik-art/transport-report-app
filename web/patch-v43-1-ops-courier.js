...
async function loadCourier(){if(loading)return loading;loading=(async()=>{const cr=(dat().routes||[]).filter(isCourier),ids=cr.map(r=>r.id),rid=ids.length?`(${ids.join(',')})`:'(0)';const dates=cr.map(r=>r.route_date).filter(Boolean).sort(),from=dates[0]||new Date().toISOString().slice(0,10),to=dates.at(-1)||from;const [s,l,c]=await Promise.all([api(`/rest/v1/courier_shipments?select=*&shipment_date=gte.${from}&shipment_date=lte.${to}&order=created_at.desc`),api(`/rest/v1/courier_shipment_points?select=*&route_id=in.${rid}`),api('/rest/v1/courier_carriers?select=*&active=eq.true&order=name')]);cdata={shipments:s||[],links:l||[],carriers:c||[]}})();try{await loading}finally{loading=false}}
...
function enhance(){addFopEditors();['fop','pickup','replen'].forEach(applyBlock);const pick=findBlock('pickup');if(pick)pick.classList.add('v431-pickup-purple');if($('.v43-stack')&&!$('#v431-courier')&&!$('#v433-courier-load-error'))renderCourier()}
